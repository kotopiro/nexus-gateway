...
- [x] `mix deps.get` の CI 経由フル検証 (GitHub Actions で確認済み)
- [x] REQUEST_MEMBERS (`GUILD_MEMBERS_CHUNK` 配信)
- [x] PostgreSQL 実スキーマ確定 (`migrations/` 参照、実DBで検証済み)
- [ ] **nexus-api (Go) の実装** — 認証トークン発行・メッセージ永続化。
      現状 gateway は DataSource.Stub で動いているだけで、本当の
      アカウント管理・サーバー間連携はまだ存在しない。次の最重要ピース。
- [ ] nexus-crypto / クライアント側 Signal Protocol・MLS の実装
      (現状 gateway は暗号化Blobを右から左に転送するだけで、
      暗号化する側=クライアントが存在しない)
...
`migrations/` 配下に golang-migrate 互換のSQLマイグレーションがある。
設計判断 (特に `channels.guild_id` の扱い) は `migrations/README.md` に
詳細を記載。実際に PostgreSQL 16 に適用し、`DataSource.Postgres` の
全関数を検証済み (`test/nexus_gateway/data_source/postgres_test.exs`,
`@tag :integration`)。
